...
namespace BexMVC.Controllers
{
    ...
    public class TrebovanjeStavkeController : Controller
    {
        public TrebovanjeStavkeController() : this(new BexUow(), new ExceptionSolver())
        { }
        public TrebovanjeStavkeController(
            IBexUow bexUow,
        ...
        [HttpGet]      
        public ActionResult GetTrebovanjeStavke(int pageSize, int pageNumber, string sortColumn, string sortOrder, string search, string searchColumn, string searchTerms)
        {

            var skip = (pageNumber - 1) * pageSize;

            var trebovanjeStavkeData = BexUow.TrebovanjeStavke.GetAll(true).Select(x =>
                                                         new TrebovanjeStavkeIndexData
                                                         {
                                                             IdStavkeTrebovanja = x.IdStavkeTrebovanja,
                                                             Trebovanje = x.TrebovanjeId,
                                                             DatumTrebovanja = x.Trebovanje.DatumTrebovanja,
                                                             Artikal = "",
                                                             Trebovano = x.KolicinaTrazena,
                                                             Lager = 0,
                                                             Izdato = x.KolicinaIzdata,
                                                             Trebuje = x.Trebovanje.Kontakt.Naziv,
                                                             Adresa = x.Trebovanje.Kontakt.AdresaTekst
                                                         }).AsEnumerable();

            if (!String.IsNullOrEmpty(searchTerms))
            {
                trebovanjeStavkeData = GetSearchTrebovanjeStavkeData(searchTerms, trebovanjeStavkeData)
                                                    .Select(x =>
                                                         new TrebovanjeStavkeIndexData
                                                         {
                                                             IdStavkeTrebovanja = x.IdStavkeTrebovanja,
                                                             Trebovanje = x.Trebovanje,
                                                             DatumTrebovanja = x.DatumTrebovanja,
                                                             Artikal = x.Artikal,
                                                             Trebovano = x.Trebovano,
                                                             Lager = x.Lager,
                                                             Izdato = x.Izdato,
                                                             Trebuje = x.Trebuje,
                                                             Adresa = x.Adresa
            ...
            var total = trebovanjeStavkeData.Count();

            if (sortColumn == "")
            {
                sortColumn = "IdStavkeTrebovanja";
                sortOrder = "desc";
            }

            if (sortOrder.Equals("desc"))
                trebovanjeStavkeData = trebovanjeStavkeData.OrderByDescending(s => s.GetType().GetProperty((sortColumn == "") ? "IdStavkeTrebovanja" : sortColumn).GetValue(s)).ToList().Skip(skip).Take(pageSize);
            else
                trebovanjeStavkeData = trebovanjeStavkeData.OrderBy(s => s.GetType().GetProperty((sortColumn == "") ? "IdStavkeTrebovanja" : sortColumn).GetValue(s)).ToList().Skip(skip).Take(pageSize);
            

            var jsonData = new TableJsonIndexData<TrebovanjeStavkeIndexData>()
            {
                total = total,
                rows = trebovanjeStavkeData
            };
        ...
        public IEnumerable<TrebovanjeStavkeIndexData> GetSearchTrebovanjeStavkeData(string searchTerms, IEnumerable<TrebovanjeStavkeIndexData> searchDataSet)
        {
            ...
            var trebovanjeStavke = searchDataSet;
