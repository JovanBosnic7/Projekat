...
{
    public class UgovorRepository : BaseRepository<Ugovor>, IUgovorRepository
    {
        public UgovorRepository(DbContext dbContext) :
        ...
        public IEnumerable<Ugovor> GetUgovorData()
        {
            var ugovor = DataSet.Include(a => a.Kontakt)
            .Include(r => r.Region)
           // .Include(s => s.Zaposleni.Kontakt)
            //.Include(f => f.KontaktIdNosilac)
            .Include(c => c.Cenovnik)
        ...
        public int GetTotalUgovor()
        {
            var ugovorQuery = DataSet.AsQueryable();

            return ugovorQuery.Count();
        }

        public IEnumerable<Ugovor> GetSearchUgovorData(string searchTerms)

        {
            ...
            string searchColumnBrojUgovora = "";
            ...
            var ugovor = DataSet.AsQueryable();
            ...
            foreach (string t in terms)
            {
                ...
                if (searchColumn.Equals("BrojUgovora") && !String.IsNullOrEmpty(searchTxt))
                {
                    searchColumnBrojUgovora = searchTxt;
                    ugovor = ugovor.Where(k => k.UgovorBroj.ToString().ToUpper().Contains(searchColumnBrojUgovora.ToUpper()));
                }
                else if (searchColumn.Equals("Nosilac") && !String.IsNullOrEmpty(searchTxt))
                {
                    searchColumnNosilac = searchTxt;
                    ugovor = ugovor.Where(k => k.NosilacNaziv.ToUpper().Contains(searchColumnNosilac.ToUpper()));
                }
                else if (searchColumn.Equals("Region") && !String.IsNullOrEmpty(searchTxt))
                {
                    searchColumnRegion = searchTxt;
