...
(function ($) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                for (var key in data) {
                                    ...
                                    addOptionToSelectControl(selectControl, data[key].Value, data[key].Value);
            ...
            if (header.find('.date-filter-control').length > 0) {
                $.each(that.columns, function (i, column) {
                    if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'datepicker') {
                        ...
                        //    });
                        header.find('.date-filter-control.bootstrap-table-filter-control-' + column.field).dateRangePicker({
                            autoClose: true
                        }).bind('datepicker-closed', function (e) {
                                //$(sprintf("#%s", e.currentTarget.id)).val(e.currentTarget.value);
                                //Fired the keyup event
                                //$(e.currentTarget).keyup();
                            //alert(column.field);
                            var izabranDatum = e.currentTarget.value;
                                //$('#' + column.field).val();
                                if (izabranDatum !== "") {
                                    $('#table').addFilter(column.field, izabranDatum);
                                    $('#table').refreshTable();
                                }
                            });
    ...
    BootstrapTable.prototype.onColumnSearch = function (event) {
        ...
        //alert('id: ' + $field + ' value: ' + text);
        if (text === "") {
            $('#table').removeFilter($field, text);
        } else {
            $('#table').addFilter($field, text);
        }
        //$('#table').addFilter($field, text);
        $('#table').refreshTable();
    };
